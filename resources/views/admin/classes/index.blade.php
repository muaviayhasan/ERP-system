...
@php use App\Http\Controllers\Admin\SchoolClassController; @endphp

@section('content')
    <div class="mb-lg flex flex-col justify-between gap-md md:flex-row md:items-end">
        <div>
            <h2 class="font-headline-lg text-headline-lg text-on-surface">Class Management</h2>
            <p class="text-body-md text-on-surface-variant">Define and manage academic classes across campuses.</p>
        </div>
        @can('classes.create')
    ...
    <form method="GET" class="mb-lg grid grid-cols-1 gap-md md:grid-cols-4">
        <div class="relative md:col-span-2">
            ...
            <input type="text" name="search" value="{{ request('search') }}" placeholder="Search by class name or code..."
        ...
        <select name="level" data-allow-clear placeholder="All Levels"
            ...
            <table class="w-full border-collapse text-left">
                <thead class="border-b border-outline-variant bg-surface-container-low">
                    <tr class="text-label-sm uppercase tracking-wider text-on-surface-variant">
                        ...
                        <th class="px-lg py-4 font-bold">Sections</th>
                    ...
                    @forelse ($classes as $class)
                        <tr class="transition-colors hover:bg-surface-container-low/40">
                            <td class="px-lg py-4">
                                ...
                                <div class="text-label-sm text-on-surface-variant">{{ $class->code }}</div>
                            </td>
                            <td class="px-lg py-4">
                                @if ($class->academic_level)
                                    <span class="rounded-md bg-secondary-container px-2 py-1 text-label-sm text-on-secondary-container">{{ $class->academic_level }}</span>
                                @else <span class="text-outline">—</span> @endif
                            </td>
                            <td class="px-lg py-4 text-on-surface-variant">{{ $class->campus?->name ?? '—' }}</td>
                            <td class="px-lg py-4 text-on-surface-variant">{{ $class->sections_count }}</td>
                            <td class="px-lg py-4">
                                <span class="inline-flex items-center gap-1 rounded-full px-2.5 py-0.5 text-label-sm font-medium {{ $class->is_active ? 'bg-tertiary/10 text-tertiary' : 'bg-outline-variant/40 text-on-surface-variant' }}">
                            ...
                            <td class="px-lg py-4">
                                <div class="flex items-center justify-end gap-1">
                                    @can('classes.edit')
                                        <a href="{{ route('classes.edit', $class) }}" class="rounded-lg p-2 text-on-surface-variant hover:bg-surface-container hover:text-primary" title="Edit">
                                            <span class="material-symbols-outlined text-[20px]">edit</span>
                                        </a>
                                    @endcan
                                    @can('classes.delete')
                                        <form method="POST" action="{{ route('classes.destroy', $class) }}" onsubmit="return confirm('Delete {{ $class->name }}?');">
                                            @csrf @method('DELETE')
